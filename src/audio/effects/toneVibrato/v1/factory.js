  ...
  const applyValue = (value) => {
    ...
    try {
      ...
      Object.entries(parameterMappings).forEach(([paramName, paramRange]) => {
        ...
        if (paramName === 'wet' || paramName === 'depth') {
          mappedValue = clamp(mappedValue, 0, 1);
        }
        if (paramName === 'frequency') {
          mappedValue = clamp(mappedValue, 0.1, 16);
        }
      ...
      console.warn('[ToneVibratoEffect] Failed to apply value', error);
    }
  };
...
export function createToneVibratoEffect({ Tone, settings } = {}) {
  if (!Tone?.Vibrato) {
    throw new Error('[ToneVibratoEffect] Tone.Vibrato constructor is required.');
  }
  ...
  const node = isSettingsArray
    ? new Tone.Vibrato(...ctorArgs)
    : new Tone.Vibrato(ctorArgs[0]);
...
export default createToneVibratoEffect;
