  ...
  const applyValue = (value) => {
    ...
    try {
      ...
      Object.entries(parameterMappings).forEach(([paramName, paramRange]) => {
        ...
        if (paramName === 'wet' || paramName === 'width') {
      ...
      console.warn('[ToneStereoWidenerEffect] Failed to apply value', error);
    }
  };
...
export function createToneStereoWidenerEffect({ Tone, settings } = {}) {
  if (!Tone?.StereoWidener) {
    throw new Error('[ToneStereoWidenerEffect] Tone.StereoWidener constructor is required.');
  }
  ...
  const node = isSettingsArray
    ? new Tone.StereoWidener(...ctorArgs)
    : new Tone.StereoWidener(ctorArgs[0]);
...
export default createToneStereoWidenerEffect;
