  ...
  const applyValue = (value) => {
    ...
    try {
      ...
      if (!segment?.parameterMappings) {
        console.warn('[ToneFrequencyShifterEffect] No segment mappings found');
        return;
      }

      // All params: continuous updates
      const frequencyRange = segment.parameterMappings.frequency;
      const wetRange = segment.parameterMappings.wet;

      const frequencyValue = frequencyRange.min + localNorm * (frequencyRange.max - frequencyRange.min);
      const wetValue = clamp(wetRange.min + localNorm * (wetRange.max - wetRange.min), 0, 1);

      // Apply all parameters continuously
      if (automationBridge?.ramp && Number.isFinite(frequencyValue)) {
        try {
          automationBridge.ramp(frequencyValue);
        } catch (err) {
          setToneValue(node, 'frequency', frequencyValue);
        }
      } else if (Number.isFinite(frequencyValue)) {
        setToneValue(node, 'frequency', frequencyValue);
      }
      
      setToneValue(node, 'wet', wetValue);
    } catch (error) {
      console.warn('[ToneFrequencyShifterEffect] Failed to apply value', error);
    }
  };
...
export function createToneFrequencyShifterEffect({ Tone, settings } = {}) {
  if (!Tone?.FrequencyShifter) {
    throw new Error('[ToneFrequencyShifterEffect] Tone.FrequencyShifter constructor is required.');
  }
  ...
  const node = isSettingsArray
    ? new Tone.FrequencyShifter(...ctorArgs)
    : new Tone.FrequencyShifter(ctorArgs[0]);
...
export default createToneFrequencyShifterEffect;
