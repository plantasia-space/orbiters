  ...
  const applyValue = (value) => {
    ...
    try {
      ...
      if (!segment?.parameterMappings) {
        console.warn('[ToneAutoWahEffect] No segment mappings found');
        return;
      }

      // All params: continuous updates (no throttling)
      const sensitivityRange = segment.parameterMappings.sensitivity;
      const baseFreqRange = segment.parameterMappings.baseFrequency;
      const octavesRange = segment.parameterMappings.octaves;
      const wetRange = segment.parameterMappings.wet;

      const sensitivityValue = sensitivityRange.min + localNorm * (sensitivityRange.max - sensitivityRange.min);
      const baseFreqValue = baseFreqRange.min + localNorm * (baseFreqRange.max - baseFreqRange.min);
      const octavesValue = octavesRange.min + localNorm * (octavesRange.max - octavesRange.min);
      const wetValue = wetRange.min + localNorm * (wetRange.max - wetRange.min);

      // Apply all parameters continuously
      if (automationBridge?.ramp) {
        automationBridge.ramp(sensitivityValue);
      } else {
        setToneValue(node, 'sensitivity', sensitivityValue);
      }
      
      setToneValue(node, 'baseFrequency', baseFreqValue);
      setToneValue(node, 'octaves', octavesValue);
      setToneValue(node, 'wet', wetValue);
    } catch (error) {
      console.warn('[ToneAutoWahEffect] Failed to apply value', error);
    }
  };
...
export function createToneAutoWahEffect({ Tone, settings } = {}) {
  if (!Tone?.AutoWah) {
    throw new Error('[ToneAutoWahEffect] Tone.AutoWah constructor is required.');
  }
  ...
  const node = isSettingsArray
    ? new Tone.AutoWah(...ctorArgs)
    : new Tone.AutoWah(ctorArgs[0]);
...
export default createToneAutoWahEffect;
