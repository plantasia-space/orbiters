...
function transformNormalized(x, transform) {
  ...
  switch (transform) {
    ...
    case 'stepLock': {
      // snapping handled separately for discrete mappings
      return n;
    }
    case 'linear':
    default:
...
function pickDiscreteOption(options, norm) {
  if (!Array.isArray(options) || options.length === 0) return null;
  const idx = Math.max(0, Math.min(options.length - 1, Math.floor(norm * options.length)));
  return options[idx];
}
...
function createModule({ node, manifest, moduleSpec }) {
  ...
  const applyValue = (value) => {
    ...
    try {
      ...
      // Start LFO on first engagement (when moving away from equilibrium)
      if (!lfoStarted && Math.abs(normalized - 0.5) > 0.01) {
      ...
      const discrete = moduleSpec.control?.discrete || {};
      
      const parameterMappings = segment.parameterMappings || {};

      Object.entries(parameterMappings).forEach(([paramName, paramRange]) => {
        if (paramName === 'frequency' && Array.isArray(discrete.frequencyDivisions)) {
          const choice = pickDiscreteOption(discrete.frequencyDivisions, primaryNorm);
          setToneValue(node, 'frequency', choice);
          return;
        }
        ...
        if (paramName === 'wet' || paramName === 'depth') mappedValue = clamp(mappedValue, 0, 1);
        if (paramName === 'spread') mappedValue = clamp(mappedValue, 0, 180);
        if (paramName === 'frequency') mappedValue = clamp(mappedValue, 0.1, 16);
      ...
      console.warn('[ToneTremoloEffect] Failed to apply value', error);
    }
  };
  ...
  return {
    ...
    configure() {
      ...
      // Initialize to bypass state (equilibrium)
      // Set wet=0 and depth=0 to ensure no modulation until user engages
      setToneValue(node, 'wet', 0);
      setToneValue(node, 'depth', 0);
      
      // DON'T start the LFO yet - it will be started when user first engages the effect
      // Starting with depth=0 can cause audio cutting issues
    },
...
export function createToneTremoloEffect({ Tone, settings } = {}) {
  if (!Tone?.Tremolo) {
    throw new Error('[ToneTremoloEffect] Tone.Tremolo constructor is required.');
  }
  ...
  const node = isSettingsArray
    ? new Tone.Tremolo(...ctorArgs)
    : new Tone.Tremolo(ctorArgs[0]);
...
export default createToneTremoloEffect;
