  ...
  const applyValue = (value) => {
    ...
    try {
      ...
      if (!segment?.parameterMappings) {
        console.warn('[ToneChebyshevEffect] No segment mappings found');
        return;
      }

      // All params: continuous updates
      const orderRange = segment.parameterMappings.order;
      const wetRange = segment.parameterMappings.wet;

      let orderValue = orderRange.min + localNorm * (orderRange.max - orderRange.min);
      orderValue = Math.round(clamp(orderValue, 1, 100));
      const wetValue = wetRange.min + localNorm * (wetRange.max - wetRange.min);

      // order regenerates the waveshaper curve — only apply when the integer value changes
      if (orderValue !== lastOrder) {
        lastOrder = orderValue;
        if (automationBridge?.ramp) {
          automationBridge.ramp(orderValue);
        } else {
          setToneValue(node, 'order', orderValue);
        }
      }
      
      setToneValue(node, 'wet', wetValue);
    } catch (error) {
      console.warn('[ToneChebyshevEffect] Failed to apply value', error);
    }
  };
...
export function createToneChebyshevEffect({ Tone, settings } = {}) {
  if (!Tone?.Chebyshev) {
    throw new Error('[ToneChebyshevEffect] Tone.Chebyshev constructor is required.');
  }
  ...
  const node = isSettingsArray
    ? new Tone.Chebyshev(...ctorArgs)
    : new Tone.Chebyshev(ctorArgs[0]);
...
export default createToneChebyshevEffect;
